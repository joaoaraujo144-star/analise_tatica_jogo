<!--
  ...
  Histórico:
    ...
    1.20 (2026-08-31) — RPE (1-10, perceção de esforço) preenchido só pelo treinador,
                         numa tabela própria (wellness_rpe) sem nenhuma policy de RLS
                         para o jogador — ver secção "Acesso do jogador" mais abaixo.
    1.21 (2026-08-31) — reordena os 5 campos do Registo de Jogo (TRACKERS em
                         js/match.js): Faltas, Perdas de Bola, Remates, Cruzamentos,
                         Cantos — afeta também o Relatório normalizado e o CSV
                         exportado, derivados do mesmo array.
    1.22 (2026-08-31) — golos passam a ligar-se a eventos do Registo de Jogo (tabela
                         goals, ver supabase/migrations/021_goals.sql) — ver "Golos
                         ligados a eventos" na secção de padrões de código, mais abaixo.
    1.23 (2026-08-31) — o destaque de evento ligado a golo passa a aparecer também no
                         próprio ponto do campo (".marker.golo"), ao vivo e no Registo
                         de Jogo normalizado — não só na linha do log.
    1.24 (2026-08-31) — golos sofridos: mesma tabela goals e a mesma ligação a eventos,
                         distinguida por "tipo" (supabase/migrations/023_goals_sofridos.sql)
                         — ver "Golos sofridos" na secção de padrões de código.
    1.25 (2026-08-31) — resultado (golos marcados x sofridos) por cima do temporizador
                         do cronómetro, calculado a partir de "goals" — ver "Resultado
                         por cima do cronómetro" na secção de padrões de código.
    1.26 (2026-08-31) — o temporizador mostrado no ecrã passa a começar em 45:00 na
                         2ª parte — só o texto, currentMinutoNoJogo() (gravado em
                         events/goals) mantém-se sem alteração, de propósito.
    1.27 (2026-08-31) — o CSV exportado ganha as secções GOLOS/GOLOS SOFRIDOS e a
                         coluna "Golo" em cada campo do Registo de Jogo — ver
                         "Exportação CSV" na secção de padrões de código.
    1.28 (2026-08-31) — popup "Quem entra?" ao marcar um titular como "Saiu" — ver
                         "Popup 'Quem entra?' (substituição)" na secção de padrões
                         de código.
    1.29 (2026-08-31) — nova coluna "Em Campo" na convocatória (isOnField()) — ver
...
- **RPE, coach-only por construção (não só por UI)**: o RPE (perceção de esforço, 1-10) vive na sua própria tabela, `wellness_rpe`, em vez de ser mais uma coluna em `wellness_responses`. A razão é o modelo de RLS ser **por linha, não por coluna** — o jogador já tem uma policy de `select` que lhe dá acesso a todas as colunas da própria linha em `wellness_responses` (`wellness_player_select`), por isso uma coluna `rpe` ali ficaria automaticamente visível a quem chamasse `select('*')` em `jogador.js`, mesmo que a interface nunca a mostrasse — esconder só na UI não seria segurança real. `wellness_rpe` tem RLS ativa e uma única policy, `wellness_rpe_team_member` (`for all`), que exige pertença a `team_members`; **não existe nenhuma policy para o jogador**, nem sequer de `select` — um jogador autenticado que tente ler esta tabela recebe sempre zero linhas. Escrito diretamente pelo treinador (sem RPC, como o resto dos dados de equipa), em dois pontos: a tabela "Wellness de hoje" do dashboard (`pages/dashboard.html`, RPE do dia, input sempre visível) e a tabela de histórico de `wellness-jogador.html` (RPE de qualquer dia já existente em `wellness_responses`, dentro do fluxo de edição "Editar"/"Guardar").
...
- **Exportação CSV**: `wireDownloadSession()` em `js/match.js` gera um único ficheiro com várias secções (`=== NOME ===`), uma por tabela relevante — sem dependências externas, só `Blob` + `URL.createObjectURL`. Inclui `GOLOS`/`GOLOS SOFRIDOS` (minuto, marcador quando há, e a cadeia de eventos que deram origem ao golo, no formato de `eventCsvLabel()` — texto simples, sem HTML, ligados por `' -> '`) e, em cada secção de campo (Faltas/Cruzamentos/etc.), uma coluna `Golo` com a mesma descrição resumida do golo a que esse clique está ligado (`goalDescByEventId`, construído a partir de `goals`/`events.goal_id` no início da função) — sem isto, a ligação evento↔golo só existia no ecrã, nunca chegava ao ficheiro exportado.
...
- **Popup "Quem entra?" (substituição)**: `showEntrouPopup()` reutiliza a mesma estrutura/CSS de `showJogadorPopup()` (`.jogador-popup`), mas com a lista filtrada por `availableSubstitutes()` (suplentes com `substituicao !== 'Entrou'`) e chips com `data-mp-id` (o id da linha em `match_players`, não `player_id` como no popup de jogador — aqui o alvo da escolha é outra linha de `match_players` a atualizar, não um `events.player_id`). Disparado só num caso: `toggle-substituicao` num titular, quando o novo valor calculado é `'Saiu'` (não ao reverter um "Saiu" para `null`, nem ao marcar um suplente como "Entrou" diretamente). `setSubstituicao(mp, valor)` centraliza a escrita (usada tanto pelo clique direto no badge como pelo `onPick` deste popup), para não duplicar o padrão "atualização otimista + `match_players.update` + `logPlayerActions`" em dois sítios.
- **Coluna "Em Campo"**: `isOnField(mp)` (`js/match.js`) é a mesma regra já usada em `onFieldMatchPlayers()` (titular sem `substituicao === 'Saiu'`, ou suplente com `substituicao === 'Entrou'`), extraída para uma função só para não duplicar a condição em dois sítios. `renderMatchPlayers()` desenha-a como `.badge-oncampo` — visualmente igual a `.badge-estado`, mas sem `data-action` nem `cursor: pointer`, porque é só um resumo calculado, nunca clicável; editar Estado ou Substituição continua a ser a única forma de a mudar. `mp.vermelho` (direto, ou por 2º amarelo — que já marca `vermelho = 1` sozinho em `toggle-amarelo2`) tira sempre o jogador de campo, mesmo que "Saiu" nunca chegue a ser marcado manualmente — um titular expulso não continua "Em campo" só porque ninguém clicou em "Saiu" à parte. `availableSubstitutes()` (popup "Quem entra?") também exclui suplentes com `vermelho`, pela mesma razão.
- **Golos ligados a eventos** (`openGoalPopup()`/`confirmGoal()`/`removeLastGoalForPlayer()`, `js/match.js`): clicar no ⚽ de um jogador já não incrementa `match_players.golo` diretamente — abre um overlay (`.goal-overlay`/`.goal-popup`, distinto do `.jogador-popup` acima: é uma escolha de vários itens com confirmar/cancelar, não um clique único) com os eventos da parte atual (todos os `tracker_id`, mais recentes primeiro, sem limite), para escolher opcionalmente quais contribuíram para o golo (ex: um cruzamento e o remate). Ao confirmar: cria a linha em `goals`, liga os eventos escolhidos (`events.goal_id`), e só depois incrementa `match_players.golo` (mantido em paralelo, é ele que continua a aparecer nos Relatórios). Clique direito/Ctrl+clique no ⚽ chama `removeLastGoalForPlayer()`, que apaga o golo mais recente desse jogador em `goals` — os eventos que lhe estavam ligados desligam-se sozinhos (`on delete set null`), sem serem apagados; se não existir nenhum golo em `goals` para esse jogador (ex: contador ajustado antes desta funcionalidade existir), só desce o número, como antes. A secção **"Golos do jogo"** (`renderGoalsList()`) mostra cada golo com a cadeia de eventos em ordem cronológica e um botão "Editar eventos" que reabre o mesmo popup pré-preenchido; no log de cada campo do Registo de Jogo, um evento já ligado a um golo fica com fundo dourado (`.log-row-golo`, aplicado quando `c.goal_id` existe — vem de graça no `select('*')`, sem pedido extra), e o próprio ponto desenhado no campo (`.marker`) ganha um anel dourado (`.marker.golo`, em `renderMarker()`) mantendo a cor X/Y de base — a cor continua a dizer o tipo, o anel diz "fez parte de um golo". Depois de qualquer alteração a golos, `reloadAllTrackers()` corre para os logs/pontos refletirem o destaque atualizado — mais simples e seguro do que tentar saber exatamente que campos foram afetados. O mesmo destaque existe no Registo de Jogo normalizado (Relatórios, pós-jogo, `loadNormalizadoReport()`) — por isso a view `events_normalizado` também expõe `goal_id` (ver `supabase/migrations/022_events_normalizado_goal.sql`), já que essa vista lê da view em vez de `events` diretamente. Os pontos ficam escondidos durante a exportação em PDF (só os mapas de calor saem), por isso o anel dourado nunca precisou de tratamento especial de `print-color-adjust`.
- **Golos sofridos**: mesma tabela `goals` e a mesma lógica de ligação a eventos, distinguida por `tipo` (`marcado` ou `sofrido`, ver `supabase/migrations/023_goals_sofridos.sql`) — um golo sofrido nunca tem `player_id`, porque a app não guarda uma lista de jogadores do adversário para escolher um "marcador". `openGoalPopup()`/`confirmGoal()` deixam de assumir sempre um jogador: passam a receber `{ mode, mp, existingGoal }` (`mode` é `'scored'` ou `'conceded'`), e o popup/`confirmGoal()` ramificam consoante o modo — mesma escolha de eventos, título e botão diferentes ("Confirmar golo sofrido"), sem tocar em `match_players.golo` nem em `mp` quando é `'conceded'`. A secção **"Golos sofridos"** (`renderConcededList()`) fica ao lado de "Golos do jogo", com um botão **"+ Golo sofrido"** próprio em vez de um ⚽ por jogador (não há convocatória do adversário para clicar), e um botão **"Remover"** direto em cada linha (`removeConcededGoal()`) — ao contrário dos golos marcados, não há linha de convocado onde fazer clique direito/Ctrl+clique para remover o mais recente. `renderGoalsList()` e `renderConcededList()` leem da mesma `goalsCache`/`goalEventsCache` (uma só chamada a `loadGoals()`), só filtram por `g.tipo` cada uma para a sua lista; a cadeia de eventos é gerada pela função partilhada `goalChainHtml()`.
- **Resultado por cima do cronómetro** (`renderScore()`, `js/match.js`): `#score-marcados`/`#score-sofridos` contam diretamente `goalsCache` (filtrado por `tipo`) — nunca um número guardado à parte, para nunca poder desalinhar das listas "Golos do jogo"/"Golos sofridos" por baixo. Chamada no fim de `loadGoals()`, por isso atualiza-se sozinho sempre que um golo é marcado, sofrido, ou removido, sem lógica própria de incremento/decremento. Os nomes das equipas (`#score-team-nos`/`#score-team-adversario`) são só texto (`currentTeam.nome`/`currentMatch.adversario`), preenchidos uma vez em `updateIndicators()` — não mudam durante o jogo.
- **Temporizador da 2ª parte começa em 45:00, só no ecrã**: `updatePeriodoTimer()` soma 45 minutos ao texto mostrado em `#periodo-timer` quando a 2ª parte está em curso (convenção do futebol — o relógio não volta a zero ao intervalo). **Isto é só cosmético**: `currentMinutoNoJogo()`, a única função que conta para gravar `minuto` em `events`/`goals`, continua relativa ao início da própria parte (0, 1, 2... até por volta de 45), sem essa soma — nunca ligar as duas. Isto mantém intactos o CSV exportado e os relatórios (que já lidam com "1ª parte" e "2ª parte" como dimensões separadas, não como um relógio contínuo de 0 a 90). Ao mexer no temporizador ou no cálculo do minuto de um evento, confirmar sempre que se está a mexer na função certa — são deliberadamente duas fontes diferentes, uma só para o ecrã.
